...
在许多语言中，开发者可以选择赋值/传递一个值的本身或它的引用。但在 JS 中，这个决定完全由值的种类决定的。这让很多来自其他语言的开发者在开始使用 JS 时感到惊讶。
...
同样，JS 根据值的类型选择是值复制还是引用复制。原始值是通过值来持有，对象是通过引用来持有。在 JS 中，无论用什么方式，都没有办法覆盖这一点。
